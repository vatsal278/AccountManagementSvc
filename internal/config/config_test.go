...
func TestInitSvcConfig(t *testing.T) {
	...
	type testCase struct {
		...
		want func(args, *sql.DB) *SvcConfig
	}
	tests := []testCase{
		{
			name: "Success",
			args: func() (*sql.DB, args) {
				...
				db, mock, err = sqlmock.NewWithDSN(":@tcp(:)/newTemp?charset=utf8mb4&parseTime=True")
				if err != nil {
					t.Fail()
				}
				mock.ExpectExec(regexp.QuoteMeta("create table if not exists ( user_id varchar(225) not null unique, account_number int AUTO_INCREMENT, income dec(18,2) DEFAULT 0.00, spends dec(18,2) DEFAULT 0.00, created_on timestamp not null DEFAULT CURRENT_TIMESTAMP, updated_on timestamp not null DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP, active_services json, inactive_services json, primary key (account_number), index(user_id) );")).WillReturnError(nil).WillReturnResult(sqlmock.NewResult(1, 1))
				return db, args{
					cfg: Config{
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			diff := testutil.Diff(got, tt.want(s, db))
			if diff != "" {
